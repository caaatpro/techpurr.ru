			...
			<ul class="slides">
              <?php foreach ( $attachments as $attachment ) : $attachment_meta = wpex_get_attachment( $attachment ); ?>
              	<li class="slide">
					...
					<?php } ?>
                        <img src="<?php echo wp_get_attachment_url( $attachment ); ?>" alt="<?php echo $attachment_meta['title']; ?>" />
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('post-entry clr'); ?>>
		...
		if( $attachments ) {
			...
			<div class="gallery-entry-slider flexslider">
				<ul class="slides">
				  <?php foreach ( $attachments as $attachment ) : $attachment_meta = wpex_get_attachment( $attachment ); ?>
					<li><img src="<?php echo wp_get_attachment_url( $attachment ); ?>" alt="<?php echo $attachment_meta['title']; ?>" /></li>
				  <?php endforeach; ?>
				</ul><!--.slides -->
			</div><!-- .single-post-slider -->
